...
ssh root@164.92.167.42 "mkdir -p SOTOdelPRIOR/apps/cocina && cd SOTOdelPRIOR/apps/cocina && find . -mindepth 1 -maxdepth 1 ! -name 'db_data' ! -name 'cocina_db_data' -exec rm -rf {} + && tar -xzvf /root/deploy_cocina.tar.gz > /dev/null && sed -i 's/\r$//' setup_remote.sh && bash setup_remote.sh"
