...
import { createCategory, deleteCategory, CategoryFormState } from '@/app/lib/actions/settings';
import { useActionState } from 'react';
import { TrashIcon, PlusIcon } from '@heroicons/react/24/outline';
import { RecipeCategory } from '@prisma/client';

export default function CategoryListClient({ categories }: { categories: RecipeCategory[] }) {
    ...
    return (
        <div className="rounded-md bg-white p-4 shadow-sm border border-gray-200">
            <h2 className="text-lg font-semibold mb-4">Categorías de Recetas</h2>

            {/* List */}
            <ul className="mb-6 divide-y divide-gray-100">
                {categories.length === 0 && <p className="text-sm text-gray-500 italic">No hay categorías definidas.</p>}
                {categories.map((cat) => (
            ...
            <form action={formAction} className="border-t pt-4">
                <label className="block text-xs font-medium text-gray-700 mb-1">Añadir Categoría</label>
